                                ...
                                <div class="rounded-xl border p-5 shadow-md w-9/12 bg-white mt-5">
                                    <div class="flex w-full items-center justify-between border-b pb-3">
                                        ...
                                        <div class="flex items-center space-x-8">
                                            <button class="rounded-2xl border bg-neutral-100 px-3 py-1 text-xs font-semibold text-gray-500">id: {{ $application->id }}</button>
                                    ...
                                    @if (!$application->answer()->exists())
                                        <div>
                                            <a href="{{ route('answers.create', ['application' => $application->id]) }}" class="bg-green-500 text-white px-4 py-2 rounded text-sm font-medium mx-3 hover:bg-green-600 transition duration-200 each-in-out">Answer</a>
                                        </div>
                                    @endif
                                    </div>
                                    @if ($application->answer()->exists())
                                        <div class="border-t mt-5 pt-5">
                                            <p class="text-sm text-indigo-500">Answer:</p>
                            ...
                            <div class='w-full max-w-lg px-10 py-8 mx-auto bg-white rounded-lg shadow-xl'>
                                <div class='max-w-md mx-auto space-y-6'>
                                    ...
                                    <form action="{{ route('applications.store') }}" method="POST" enctype="multipart/form-data">
                                        ...
                                        <textarea name="message" id="" class="p-3 mt-2 mb-4 w-full bg-slate-200 rounded" rows="5" required></textarea>
